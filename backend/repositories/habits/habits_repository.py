...
async def archive_habit(
    ...
    """
    Перемещает активную привычку пользователя в архив.

    При архивации:
    - привычка становится архивной;
    - сохраняется точное время архивации;
    - история подтверждений не удаляется.

    Возвращает:
        True  — привычка успешно архивирована.
        False — привычка не найдена,
                принадлежит другому пользователю
                или уже находится в архиве.
    """
...
# =========================================================
# ПОЛУЧИТЬ АРХИВНЫЕ ПРИВЫЧКИ ПОЛЬЗОВАТЕЛЯ
# =========================================================
...
async def get_archived_habits(
    ...
    Для каждой привычки возвращаем:
    - основные данные привычки;
    - дату архивации;
    - количество подтверждений;
    - максимальную серию за всю историю.

    Активные привычки сюда не попадают.
    """

    async with get_connection() as connection:
        # -------------------------------------------------
        # Получаем сами архивные привычки.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Получаем всю историю подтверждений
        # только архивных привычек пользователя.
        #
        # Историю не удаляем и не меняем.
        # Она нужна для:
        # - общего количества подтверждений;
        # - максимальной серии.
        # -------------------------------------------------
    ...
    # =====================================================
    # СОБИРАЕМ ДАТЫ ПОДТВЕРЖДЕНИЙ ПО ПРИВЫЧКАМ
    # =====================================================
...
async def restore_habit(
    ...
    и сразу возвращает её полное актуальное состояние.

    После восстановления frontend получает:
    - основные данные привычки;
    - completed_today;
    - xp_awarded_today;
    - xp_amount_today;
    ...
    Дополнительный GET /api/habits не требуется.
    """
    ...
    async with get_connection() as connection:
        async with connection.transaction():
            ...
            # -------------------------------------------------
            # Вся история подтверждений привычки
            # -------------------------------------------------
            ...
            completed_dates = [
                ...
                for item in completed_date_rows
            ]
